...
<body>
	<?php
	require "fun.php";
	$sql="select * from szsj";//得到查询语句
	$result=mysqli_query($conn,$sql);
	$total=mysqli_num_rows($result);//结果集中行的数目
	$page=isset($_GET['page'])?intval($_GET['page']):1;//获取地址栏中page的值，不存在设为1
	$num=12;//每页显示12条记录
	$bothNum=4;
	$url='ShuiTiShuJu.php';//本页url
	...
	if($new_row=mysqli_fetch_array($new_result)){
		echo "<br><center><font size=8 face=楷体_GB2312 color=#008000>水稻生长信息——水体环境</font></center>";
	...
	do
	{
		list($sj,$stwd,$ph,$hzd,$sw)=$new_row;
	...
	echo "<script>alert('无记录！');location.href='ShuiTiShuJu.php';</script>";
	?>
</body>
</html>
